<template>
  ...
  <v-row >
          ...
          <v-col cols="7" class="text-center"><h1>Todas las canciones de <strong>{{$route.params.artista}}</strong></h1></v-col>
          <v-col cols="1"></v-col>
        </v-row>
        </div>
      <div>
...
this.$router.push({ path:`/generos/${usuario}`});
    ...
    volver() {
      ...
      async getCanciones() {
        console.log('hola')
        const artista = this.$route.params.artista
        console.log(artista)
        axios
          .get(`http://localhost:3000/artistaCancion/${artista}`)
          .then((response) => {
            console.log(response);
