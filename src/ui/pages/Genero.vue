<template>
  ...
  <v-row >
          ...
          <v-col cols="7" class="text-center"><h1>Todas las canciones {{ $route.params.genero }}</h1></v-col>
          <v-col cols="1"></v-col>
        </v-row>
        </div>
      <div>
...
this.$router.push({ path:`/generos/${usuario}`});
    ...
    volver() {
      ...
      async getCanciones() {
        console.log('hola')
        const genero = this.$route.params.genero
        axios
          .get(`http://localhost:3000/generoCancion/${genero}`)
          .then((response) => {
            console.log(response);
